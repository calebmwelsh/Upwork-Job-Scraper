...
- Environment variables, api tokens, etc are stored in `.env/` directory
...
- `.env/` - **Centralized credentials directory** containing:
  - `.env` - Environment variables and API keys
  - `client_secret.json` - Google OAuth client credentials
  - `token.json` - Google OAuth access/refresh tokens
  - All credential files live here and the `.env/` directory should be in `.gitignore`
